...
* the `join($joinableExtractor, $joiningFromJoinableExtractor)` methods adds an extractor that will perform a join operation upon another extractor's records, later referred as Joiners
...
* the `transform($trasformer)` method adds a transformer to the flow that will transform each record one by one, later referred as Transformer
...
* the `branch($yaEtlWorkflow)` method adds an entire flow in the flow, that will be treated as a single node and later referred as Brancher
