...
[![Build Status](https://travis-ci.org/fab2s/YaEtl.svg?branch=master)](https://travis-ci.org/fab2s/YaEtl) [![License](https://poser.pugx.org/fab2s/nodalflow/license)](https://packagist.org/packages/fab2s/yaetl)
...
Being Nodal makes it possible to chain arbitrary number of Extract to Load operations which may go through arbitrary number of transform, joins and, to even branch the workflow in case some Loaders require different transformation and or joins before hey can do their work.
...
This is required because YaEtl depends on another "unstable" (even though, test are showing that something is working) package which is not explicitly required by the project using YaEtl.

## Usage

### A fluent grammar
YaEtl can build complex and repeatable workflows very simply:
* the `from(ExtractorInterface $extractor, ExtractorInterface $aggregateWith = null)` method adds an extractor as a source of records in the flow, which may or may not be aggregated with another one and later referred as Fromer
...
* the `join(JoinableInterface $extractor, JoinableInterface $joinFrom, OnClauseInterface $onClause)` methods adds an extractor that will perform a join operation upon another extractor's records, later referred as Joiners
    ...
    The nature of the join is defined by `$onClause` which implements `OnClauseInterface`:
    ```php
    $joinOnClause  = new OnClause('fromKeyAlias', 'joinKeyAlias', function ($upstreamRecord, $record) {
        return array_replace($upstreamRecord, $record);
    });

    $leftJoinOnClause  = new OnClause('fromKeyAlias', 'joinKeyAlias', [$suitableObject, 'suitableMethod], $defaultRecord);
    ```
...
$joinOnClose = new OnClose('fromKeyAlias', 'joinKeyAlias', function($upstreamRecord, $record) {
    return array_replace($upstreamRecord, $record);
});
// or if we want a left join instead
$leftJoinOnClose new OnClose('fromKeyAlias', 'joinKeyAlias', $aSuitableCallable, [
...
YaEtl will call `getTraversable()` and iterate on it _while_ `extract()` returns true.
...
YaEtl comes with many partial to complete Extractor implementations to address most use case with some emphasis on databases in general and [PDO](http://php.net/PDO) in particular:
...
YaEtl also includes a ready to use `CallableTransformer` wich takes a `callable` as argument:
```php
...
All this means that while implementing ETL flows, you create other opportunities either withing or without the flow which will save more and more time over time, as long as you need ETLs.

And in fact, the overhead of doing so is very small, especially if your extractors is a [`Generator`](http://php.net/Generator) yielding values and are using [`SplDoublyLinkedList`](http://php.net/SplDoublyLinkedList) as record buffer as it is the case for every extractor in YaEtl when traversed. Extractor joining against another one are using an associative array to be faster on key testing, but they are not traversed as they join their data on each record one by one.
...
YaEtl also extends two protected method from NodalFlow that will be triggered just before and after the flow's execution, `flowStrat()` and `flowEnd($success)`. You can override them to add more logic. These are not treated as events as they are always used by YaEtl and NodalFlow to provide with basic statistics.
...
* Flow is clean, that is if everything went well up to this point:
    ...
    ```
* Flow is dirty, that is if the flow was broken by a node:
    ```php
...
$dbExtractor->setExtractQuery($extractQuery)->setLimit(5000);
...
In case you need to extract a lot (I mean a lot) of data at once, you could implement a direct access to PDO to fetch them.
If you are using mysql, you can also deactivate buffered queries.

**Table with unique Key extractor**
In many simple case you can use the handy `UniqueKeyTableExtractor`
...
// instantiate the generic db extractor from user table using
// default connection ordered by the implicit primary key "id"
// Note that UniqueKeyExtractor also supports composite (primary|unique) keys
$uniqueKeyExtractor = new UniqueKeyExtractor('user');
